...
namespace Points.Tools
{
	...
	public static class PointIO
	{
				...
				if (FileManager.FileExists(filePath))
				{
					using (var reader = File.OpenText(filePath))
					{
						while (!reader.EndOfStream)
						{
							...
							if (sDataLength != 4)
							{
								ThrowLoadError(item, sDataLength > 4 ? "Too many ':' splitters." : "Not enough ':' split data.");
								continue;
							}
		...
		private static void ThrowLoadError(string item, string message = "") => Log.Error($"Could not load line: [{item}] ({message})");
